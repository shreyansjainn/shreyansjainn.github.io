...
For example, Superposition as a concept was first studied in Toy models of Superposition paper, which then led to the creation of Sparse AutoEncoders and today SAEs are an integral part of understanding the types of features a model is learning.

By trying to understand feature geometries in toy models, we try to build a strong intuition on how and when features are arranged in different geometries and the types of mechanisms governing a given geometry.
...
At the start, when we compare the feature norms, there is a striking difference in the first glance itself. the overall norm in the hexagon geometry crosses the theoretical limit of 2.5 and stretches to 3 while the one for pentagon plateaus at 2.5.
...
In the case of pentagon geometry, the gradient updates for two features (dark blue and red) start at a very low point and stay low throughout the training run while for others it decreases as we go ahead in the training process. When we compare these against the per-feature losses, we observe that one feature is starting from a point of very loss (very near to the optimal point) so it doesn’t need big gradient updates, but the other one is starting from a very high loss (very far from the optimal point) and still has low gradient updates indicating that its stuck in a saddle point and is unable to escape.
...
When we observe similar plots for hexagon geometry, it shows that the gradient updates never plateaus for any feature and the losses for all the features converge to a optimal minima as well.
...
In this analysis, we try to understand the core intuition behind the feature geometry in toy models with hidden_dim=2 and validate the findings experimentally. We also try to understand in what cases the experimental results deviate from the theoretical foundation and analyse the impact of Initialization and Optimizer on the resulting feature geometry.
